...
require 'spec_helper'
require_relative '../../../lib/serasa_experian/individual/report'
require_relative '../../../lib/serasa_experian/client'

RSpec.describe SerasaExperian::Individual::Report do
  ...
  describe '#fetch' do
    context 'when the request is successful' do
      before do
        stub_request(:get, 'https://mock-api.serasaexperian.com.br/credit-services/person-information-report/v1/creditreport')
          .with(
            query: hash_including(
              reportName: 'BASIC_REPORT',
              optionalFeatures: 'FEATURE_A,FEATURE_B',
              reportParameters: Base64.strict_encode64({ reportParameters: [{ name: 'SCORE',
                                                                              value: 'HRLN' }] }.to_json)
            ),
            headers: {
              'Authorization' => 'Bearer mock_access_token',
              'X-Document-Id' => '12345678000',
  ...
  describe '#format_report_parameters' do
    ...
    context 'When there are parameters' do
      it 'encodes parameters in Base64 JSON format' do
        parameters = [{ name: 'SCORE', value: 'HRLN' }]
  ...
  describe '#sanitize_document' do
    context 'formats the document correctly' do
      it 'removes non-numeric characters from document' do
        result = report_service.send(:sanitize_document, '123.456.780-00')
        expect(result).to eq('12345678000')
      end
